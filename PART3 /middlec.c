...
int main(void)
{
  ...
  {
      float theta = (2.0f * PI * i) / SAMPLES;
      float s = sinf(theta);                  
      SineTable[i] = (uint16_t)( (s * 2047.0f) + 2048.0f );
  }
  uint32_t pclk1 = HAL_RCC_GetPCLK1Freq();   
  uint32_t psc   = htim6.Init.Prescaler + 1; 
  uint32_t arr   = htim6.Init.Period + 1;    
  uint32_t Fs    = pclk1 / psc / arr;        
  float     fout = (float)Fs / (float)SAMPLES;
  printf("Fs=%lu Hz, f_out≈%.2f Hz\r\n", (unsigned long)Fs, (double)fout);
